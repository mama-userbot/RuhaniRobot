...
def info(update: Update, context: CallbackContext):
    ...
    text = (f"╒═══「<b> Getting Information:</b> 」\n"
    ...
    if user_id not in [bot.id, 777000, 1087968824]:
        ...
        text += f"\n\n<b>Activeness:</b> <code>{userhp['earnedhp']}/{userhp['totalhp']}</code>\n[<i>{make_bar(int(userhp['percentage']))} </i>{userhp['percentage']}%]"
    ...
    if user.id == OWNER_ID:
        ...
        text += "\n\nThis user is member of 'Team Kws'."
        disaster_level_present = True
    ...
    if disaster_level_present:
        text += ' [<a href="https://t.me/melody_music_zone/16">?</a>]'.format(
            bot.username)
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "Erm... yeah, I only trust Team Kws to set my bio.")
            return
...
*What is that health thingy?*
 Come and see [HP System explained](https://t.me/melody_music_zone/16)
"""
